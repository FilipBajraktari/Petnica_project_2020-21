...
def load_data_shared():
    ...
    validation_data = (training_data[0][50000:],
                        training_data[1][50000:])
    training_data = (training_data[0][:50000],
                        training_data[1][:50000])

    training_data = (training_data[0].reshape((50000,784))/256.0, 
                        training_data[1].reshape((50000,)))
    validation_data = (validation_data[0].reshape((10000,784))/256.0, 
                        validation_data[1].reshape((10000,)))
    test_data = (test_data[0].reshape((10000,784))/256.0, 
                    test_data[1].reshape((10000,)))
...
class ConvPoolLayer(object):
    ...
    def set_inpt(self, inpt , inpt_dropout, mini_batch_size):
        ...
        self.output = self.activation_fn(pool_2d)
...
class FullyConnectedLayer(object):
    ...
    def __init__(self, n_in, n_out, activation_fn = sigmoid, p_dropout = 0.0):
        ...
        #loc je srednja vrednost gausove distribucije
        #scale je standardna devijacija
        self.w = theano.shared(
            np.asarray(
                np.random.normal(
                    loc = 0.0, 
    ...
    def set_inpt(self, inpt, inpt_dropout, mini_batch_size):
        ...
        self.output = self.activation_fn(
            (1.0-self.p_dropout)*T.dot(self.inpt, self.w) + self.b)
...
def dropout_layer(layer, p_dropout):
    ...
    mask = srng.binomial(n=1, p=1.0-p_dropout, size=layer.shape)
